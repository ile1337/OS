...
public class TancSoStudentite {
    ...
    static Semaphore accsessDancefloor;//dozvola za tancuvanje
    static Semaphore lock;//semafor za zenskite otkako ke se presoblecat da dadat znak deka se spremni za tanc
    static Semaphore canPresobleceM;//dozvoluva 10 maski da vlezat vo soblekuvalnata
    static Semaphore canPresobleceZ;//dozvoluva 10 zenski da vlezat vo soblekuvalnata
    public int numPP = 0;

    public void init() {
        //TODO: da se implementira
        accsessDancefloor = new Semaphore(3);//dozvoluva samo 3 para na dueti
        lock = new Semaphore(0);//ovoj semafor ima i uloga na tajmer se ceka se dodeka ne se pojavi barem edno zensko za tancuvanje
        canPresobleceM = new Semaphore(10);//10 maski
        canPresobleceZ = new Semaphore(10);//10 zenski
    ...
    class Masko extends Thread {
        ...
        public void ucestvo() throws InterruptedException {
            //TODO: da se implementira
            canPresobleceM.acquire();//cekaj da dobies dozvola za soblekuvalna 10 kopii od nea
            show.presobleci();
            canPresobleceM.release();//oslobodi gi site 10 kopii od soblekuvalnata
            // samo maskoto go povikuva metodot tancuvaj
            accsessDancefloor.acquire();//cekaj Dance floor da go dobies
            lock.acquire();//cekaj zensko da se presoblece vo par da moze da vleze so masko
            show.tancuvaj();
            accsessDancefloor.release();//oslobodi gi site dueti na Dance floor
        }
    ...
    class Zensko extends Thread {
        ...
        public void ucestvo() throws InterruptedException {
            //TODO: da se implementira
            canPresobleceZ.acquire();//cekaj da dobies dozvola za soblekuvalnata
            show.presobleci();
            canPresobleceZ.release();//oslobodi gi site 10 kopii od soblekuvalnata
            lock.release();//dadi znak deka ima slobodna zena za duet
